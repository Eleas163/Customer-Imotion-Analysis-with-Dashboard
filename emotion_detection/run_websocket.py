...
from emotion_detection import EmotionDetect
...
async def facedetect(websocket, path):
    while True:
        frame = vs.read()
        ...
        for face in faces:
            ...
            await websocket.send(f"{emotion}-{e_confidence*100}")
...
start_server = websockets.serve(facedetect, "127.0.0.1", 8080)
